...
export class CustomerAppointmentsComponent {
  ...
  readonly availableSlotsLabel = computed(() =>
    this.timeSlots().length
      ? `${this.timeSlots().filter((slot) => !this.isSlotUnavailable(slot)).length} open slots ready to book`
      : 'Choose a vet and date to load slots',
  );
  ...
  loadSlots() {
    ...
    forkJoin({
      ...
      unavailableSlots: this.visits.getUnavailableSlots(vetId, visitDate),
    }).subscribe({
  ...
  isSlotUnavailable(slot: string) {
    const unavailableSlots = this.unavailableSlots();
    return unavailableSlots.includes('ALL_DAY') || unavailableSlots.includes(slot);
  }
  ...
  bookAppointment() {
    ...
    if (this.isSlotUnavailable(this.form.controls.timeSlot.value)) {
      this.errorMessage.set('That slot is no longer available. Please choose another slot.');
      return;
    }
