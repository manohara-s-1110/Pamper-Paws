...
import com.pamperpaw.visit.repository.VetLeaveRepository;
...
public class VisitServiceImpl implements VisitService {
    ...
    public VisitResponseDTO createVisit(VisitRequestDTO dto) {
        ...
        validateBookableSlot(dto.getVetId(), dto.getVisitDate(), dto.getTimeSlot());
    ...
    private VisitResponseDTO mapToResponse(Visit visit) {
        ...
        response.setStatus((visit.getStatus() == null ? VisitStatus.PENDING : visit.getStatus()).name());
        return response;
    }
    ...
    public List<String> getUnavailableSlots(Long vetId, String date) {
        if (vetLeaveRepository.existsByVetIdAndDate(vetId, date)) {
            return List.of("ALL_DAY");
        }
    ...
    @Override
    public VetLeaveResponseDTO addVetLeave(VetLeaveRequestDTO dto) {
        validateReferencesForVet(dto.getVetId());

        vetLeaveRepository.findByVetIdAndDate(dto.getVetId(), dto.getDate())
                .ifPresent(existingLeave -> {
                    throw new IllegalStateException("Leave already marked for this date");
                });
        ...
        return mapLeaveToResponse(vetLeaveRepository.save(leave));
    }

    @Override
    public List<VetLeaveResponseDTO> getVetLeaves(Long vetId) {
        return vetLeaveRepository.findByVetIdOrderByDateAsc(vetId)
                .stream()
    ...
    @Override
    public VisitResponseDTO updateVisitStatus(Long id, String status) {
        Visit visit = visitRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Visit not found with id: " + id));

        try {
            visit.setStatus(VisitStatus.valueOf(status.trim().toUpperCase()));
        } catch (IllegalArgumentException ex) {
            throw new IllegalStateException("Status must be COMPLETED, MISSED, or PENDING");
        }

        return mapToResponse(visitRepository.save(visit));
    }

    private void validateBookableSlot(Long vetId, String visitDate, String timeSlot) {
        if (vetLeaveRepository.existsByVetIdAndDate(vetId, visitDate)) {
            throw new IllegalStateException("Vet is on leave for this date");
        }
    ...
    private void validateReferencesForVet(Long vetId) {
        try {
            vetClient.getVetById(vetId);
        } catch (Exception ex) {
            throw new ResourceNotFoundException("Vet not found with id: " + vetId);
    ...
    private VetLeaveResponseDTO mapLeaveToResponse(VetLeave leave) {
        VetLeaveResponseDTO response = new VetLeaveResponseDTO();
        response.setId(leave.getId());
        response.setVetId(leave.getVetId());
